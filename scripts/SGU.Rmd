...
dfEpisodeData <- select(dfEpisodeData, c(1,2,7,8)) %>% filter(Episode %in% Episode_range)
...
# Overall Performance
winsOverallByPanelist <- dfPanelistPerf %>% group_by(Panelist) %>% summarise(Overall =  sum(Correct)/n(), CorrectGuesses = sum(Correct), TotalGuesses=n())
winsOverallByPanelist <- as.data.frame(winsOverallByPanelist)

winsOverallTotal <- dfOverallPerf %>% summarise(Overall =  sum(Correct)/n(), CorrectGuesses = sum(Correct), TotalGuesses=n())
...
winsByTheme <- dfPanelistPerf %>% group_by(Panelist, Theme) %>% summarise(pctThemes = sum(Correct)/n(), CorrectGuesses = sum(Correct), TotalGuesses=n())



winsByThemeTotal <- dfOverallPerf %>% group_by(Theme) %>% summarise(pctThemes = sum(Correct)/n(), CorrectGuesses = sum(Correct), TotalGuesses=n()) %>% mutate(Panelist = 'ALL') %>% select(Panelist, Theme, pctThemes, CorrectGuesses, TotalGuesses)
...
p_final <- ggplot(summary_final, aes(x=Panelist, y=Statistic)) + geom_bar(aes(fill=Result),stat="identity", position = position_dodge(width=.85), color="grey40", width=0.80) + ggtitle("Evan Leads Overall \n Cara Leads on Segments with Themes \n Evan Leads on Segments Without Themes") + 
     ...
     geom_text(aes(group=Result, label=percent(Statistic,.1)), vjust=2, position = position_dodge(width=.85), size=6) +
