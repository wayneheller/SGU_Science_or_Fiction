...
p_final <- ggplot(summary_final, aes(x=Panelist, y=Statistic)) + geom_bar(aes(fill=Result),stat="identity", position = position_dodge(width=.85), color="grey40", width=0.80) + ggtitle("Evan Leads Overall \n Cara Leads on Segments with Themes \n Evan Leads on Segments Without Themes") + 
...
For each combination, the number of times this combination has occurred, the total correct answers, total possible answers, and the accuracy percentage (correct/possible) is listed below.

``` {r AnsweringOrder, echo = FALSE}
dfAnsweringOrderPerf <- dfEpisodeData%>% group_by(FirstPanelist, SecondPanelist, ThirdPanelist, FourthPanelist, FifthPanelist)  %>% summarise(Occurred = sum(n()), TotalCorrect = sum(CorrectAnswers), TotalAnswers = sum(TotalPanelists), PercentCorrect = TotalCorrect / TotalAnswers * 100) %>% arrange(desc(PercentCorrect), desc(Occurred))

names(dfAnsweringOrderPerf) <- c('First', 'Second', 'Third', 'Fourth', 'Fifth', 'Frequency', 'Correct', 'Possible','Accuracy(%)')
kable(dfAnsweringOrderPerf, format.args = list(justify='right', width=500) , rownames=FALSE, digits=1, align = c('l', 'l', 'l','l','l','c','c','c', 'r'),format='html',table.attr = "style='width:75%;'")
```
