...
dfAnsweringOrderPerf <- dfEpisodeData%>% group_by(FirstPanelist, SecondPanelist, ThirdPanelist, FourthPanelist, FifthPanelist)  %>% summarise(Occurred = sum(n()), TotalCorrect = sum(CorrectAnswers), TotalAnswers = sum(TotalPanelists), TotalPanelistSweeps = sum(PanelistSweep), PercentCorrect = TotalCorrect / TotalAnswers * 100) %>% arrange(desc(PercentCorrect), desc(Occurred))

names(dfAnsweringOrderPerf) <- c('First', 'Second', 'Third', 'Fourth', 'Fifth', 'Frequency', 'Correct', 'Possible','Sweeps', 'Accuracy(%)')
kable(dfAnsweringOrderPerf %>% filter(Frequency >=3), format.args = list(justify='right', width=500) , rownames=FALSE, digits=1, align = c('l', 'l', 'l','l','l','c','c','c','c', 'r'),format='html',table.attr = "style='width:75%;'")
```
##### Answering Order Combinations Full Detail
``` {r AnsweringOrderUnfiltered, echo = FALSE}
kable(dfAnsweringOrderPerf, format.args = list(justify='right', width=500) , rownames=FALSE, digits=1, align = c('l', 'l', 'l','l','l','c','c','c','c', 'r'),format='html',table.attr = "style='width:75%;'")
```
